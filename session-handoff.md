...
>
> **TRẠNG THÁI MỚI NHẤT (S32, 22/06 — thay cho phần dưới):** `origin/Dat` = **`c25f38b`+docs** · `origin/main` = **`121d0e0`** (A7+A8 LIVE; **B7+C1+C2 chưa deploy**). Suite **45 passed / 1 skipped / 1 xfailed**; spec **40 — 37 active / 1 gap / 2 planned**. 🎉 Track A + Track B + auth + A7/A8 + B7 + **C1** (API nộp bài+chấm, SPEC-SUBMIT-001) + **C2** (UI thí sinh làm bài) HOÀN TẤT. 🏁 **TOEIC end-to-end ĐỦ: tạo đề → phát hành → thí sinh đăng nhập làm bài → nộp → chấm điểm tự động** (verify LIVE: backend đề 200 câu toàn-đúng=990; UI trả lời 3/200→10đ chứng minh payload đủ). **Chi tiết S25-S32 ở `claude-progress.md`.**
> **Kế tiếp gợi ý:** **gộp deploy B7+C1+C2 lên `main`** (giờ đủ backend chấm + UI làm bài → đáng đưa LIVE; lưu ý thí sinh cần tài khoản — UI đăng ký tự phục vụ là follow-up nhỏ nếu cần). Sau: **GRADE-002** (cần Đạt duyệt thêm cột `score_listening`/`score_reading` vào model Grade đóng băng). **Hoãn:** VSTEP/HSK · giám thị webcam/tab (`exam-room-ui`, đã có hooks) · chấm tự luận (GRADE-003 Celery) · SCALE-003.
> ⚠️ **Anti S32 lặp lỗi ranh giới 2 lần** (C1 và C2): đều tự sửa `claude-progress.md`/`session-handoff.md` (file Claude — brief cấm; C1 còn xoá Next Steps lịch sử + tự viết "Claude verified" trước khi verify) → Claude khôi phục baseline + tự viết lại cả 2 lần. Code C1/C2 thì sạch, đúng phạm vi. *(memory `anti-recurring-slips`.)*
