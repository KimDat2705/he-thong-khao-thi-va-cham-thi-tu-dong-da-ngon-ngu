...
> **Kế tiếp gợi ý:** **🚀 gộp deploy C4+C5+GRADE-002 lên `main`** (cần Đạt xác nhận deploy minh bạch; ⚠️ lưu ý migration cột mới trên DB Render — nếu DB cũ còn giữ thì cần `alembic upgrade head` hoặc clear DB để create_all tạo cột). **Hoãn:** VSTEP/HSK · giám thị webcam/tab (`exam-room-ui`, đã có hooks) · chấm tự luận (GRADE-003 Celery) · SCALE-003.
