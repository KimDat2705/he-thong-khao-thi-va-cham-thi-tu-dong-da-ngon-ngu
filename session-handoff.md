# Session Handoff — cập nhật 22/06/2026 (Claude — 🎉 C1/C2/C3: LUỒNG THÍ SINH TOEIC TRỌN VẸN)
...
>
> **TRẠNG THÁI MỚI NHẤT (S32, 22/06 — thay cho phần dưới):** `origin/Dat` = **`114387f`+docs** · `origin/main` = **`121d0e0`** (A7+A8 LIVE; **B7+C1+C2+C3 chưa deploy**). Suite **45 passed / 1 skipped / 1 xfailed**; spec **40 — 37 active / 1 gap / 2 planned**. 🎉 Track A + Track B + auth + A7/A8 + B7 + **C1** (API nộp bài+chấm, SPEC-SUBMIT-001) + **C2** (UI làm bài) + **C3** (cổng thí sinh: /exams + /register + định tuyến vai trò) HOÀN TẤT. 🏁 **Luồng thí sinh TOEIC TRỌN VẸN: đăng ký → đăng nhập → /exams → làm bài → nộp → chấm điểm tự động** (verify LIVE: backend 200 câu toàn-đúng=990; UI candidate→/exams, admin→/admin). **Chi tiết S25-S32 ở `claude-progress.md`.**
> **Kế tiếp gợi ý:** **🚀 gộp deploy B7+C1+C2+C3 lên `main`** — giờ đủ điều kiện đưa LIVE trọn luồng (cần Đạt xác nhận deploy minh bạch; thí sinh tự đăng ký được nên không cần seed account). Sau: **GRADE-002** (cần Đạt duyệt thêm cột `score_listening`/`score_reading` vào model Grade đóng băng). **Hoãn:** VSTEP/HSK · giám thị webcam/tab (`exam-room-ui`, đã có hooks) · chấm tự luận (GRADE-003 Celery) · SCALE-003.
> ⚠️ **Anti S32 lặp lỗi ranh giới 3 lần** (C1/C2/C3): đều tự sửa `claude-progress.md`/`session-handoff.md` (file Claude — brief cấm; C1 còn xoá Next Steps lịch sử + tự viết "Claude verified"; C3 tự ghi "Anti tuân thủ đúng ranh giới") → Claude khôi phục baseline + tự viết lại cả 3 lần. Code C1/C2/C3 thì sạch, đúng phạm vi. *(memory `anti-recurring-slips` — Anti vẫn lặp lỗi này kể cả sau nhiều lần nhắc.)*
