...
9. ✅ **SLICE W2 (viết thư) XONG** (02/07 S53, SPEC-FACTORY-007): **FORMAT THỨ 7**, KHÔNG có đáp án (tự luận). GROUND trên w2 THẬT (vai + bối cảnh thư nhận + 2-3 ý + "~100 words"). `load_w2_seeds`/`build_w2_variants` (`w2_item` {role,situation,points,instruction,domain_guess} + `w2_raw`, không key)/`qc_w2` + CLI + test. pytest 68→**69/0**; verify Gemini THẬT (thư "flu" → đề "nấu ăn"). Review 8 agent → vá 3 defect (points scalar crash; junk points; near-dup vs seed).
10. ✅ **SLICE NGHE XONG** (02/07 S53, SPEC-FACTORY-008; **D3 chốt hướng (b)** PoC TTS): **FORMAT THỨ 8**, format Cambridge PET (L1 5 chọn-tranh hội thoại 2 người + L2 10 điền-từ). Nghiên cứu web/GitHub (Podcastfy, Gemini multi-speaker native ≤2 giọng, tapescript PET đọc-2-lần, ghép audio stdlib). SINH-NGƯỢC-TỪ-ĐÁP-ÁN (pool_lis không có transcript): sinh kịch bản + VALIDATE đáp án L2 ∈ transcript + gate số-thứ-tự gap. Audio: đa-giọng MultiSpeakerVoiceConfig + ghép wave (WAV). pytest 69→**71/0**; verify Gemini THẬT build WAV 137.6s đa-giọng. Review 13 agent → vá 4 nhóm defect.
11. 🎉 **HOÀN TẤT 8/8 DẠNG NGÂN HÀNG B1** (R1/R2/R3/R4 Đọc + W1/W2 Viết + Nói P3 + Nghe P4). **THỨ TỰ TIẾP**:
   - **Nghe hardening/đóng gói**: ghép TRỌN bài 17' (15 câu 1 file) + đọc-lại toàn phần; xuất **MP3** (lameenc, không ffmpeg); sinh ảnh L1 thật; **GV nghe duyệt** audio máy (timbre 2 giọng).
   - **Helper render-từ-JSON tham chiếu** (de-risk D2 điểm 2) cho part text.
   - Chốt **D4** (GV ký + soát chéo đa-agent) / **D5** (sản lượng); xin sếp 1 record mẫu `bank_raw` + xác nhận D2 (a). Ảnh R2/R3 tách slice riêng.
12. Mỗi slice: spec `SPEC-FACTORY-00N` + test (mock tất định) + verify dữ liệu thật + review đối kháng + cập nhật harness (như quy trình BANK-005/6/7).
