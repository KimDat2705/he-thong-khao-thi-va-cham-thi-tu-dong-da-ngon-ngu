...
8. **THỨ TỰ TIẾP** (đúng logic + tối ưu):
   - **W1** (viết lại câu giữ nghĩa, có câu mẫu = key) → **W2** (đề thư ~100 từ theo 14 chủ đề, KHÔNG key — tự luận, chấm AI/GV).
...
9. Mỗi slice: spec `SPEC-FACTORY-00N` + test (mock tất định) + verify dữ liệu thật + review đối kháng + cập nhật harness (như quy trình BANK-005/6/7).
