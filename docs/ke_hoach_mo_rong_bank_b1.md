...
10. 🎉 **HOÀN TẤT 7/7 DẠNG BÀN GIAO** (R1/R2/R3/R4 Đọc + W1/W2 Viết + Nói). **THỨ TỰ TIẾP**:
   - **Nghe (P4): HOÃN** — cần audio/TTS; sếp bảo audio là việc trung tâm/người → chốt **D3** (hoãn hẳn hay PoC TTS) trước.
   - **Helper render-từ-JSON tham chiếu** (de-risk D2 điểm 2) — làm khi có nhịp, không chặn.
   - Chốt **D3/D4/D5** với sếp; xin 1 record mẫu `bank_raw` hoàn chỉnh để đối chiếu đủ cờ build_db.
11. Mỗi slice: spec `SPEC-FACTORY-00N` + test (mock tất định) + verify dữ liệu thật + review đối kháng + cập nhật harness (như quy trình BANK-005/6/7).
