...
from app.services import boss_pipeline, boss_render  # noqa: E402
...
def main():
    ...
    bundles = boss_pipeline.run_bank_expansion(
        ...
        per_seed=args.per_seed, generator=gen, limit=args.limit)

    os.makedirs(args.out, exist_ok=True)
    report = boss_pipeline.roundtrip_report(bundles, args.n_target)
    print(f"\nSinh {len(bundles)} dạng. Tổng item merge-ready: {report['total_merge_ready']} · all_ok={report['all_ok']}")
    for skill, bundle in bundles.items():
        ...
        print(f"  [{skill}] {bundle['count']} item · QC {bundle['count_qc_ok']} · merge-ready "
              f"{rt['count_merge_ready']} · {rt['by_difficulty']} · đủ-pool={rt['enough_for_n']}"
